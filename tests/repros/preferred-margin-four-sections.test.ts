...
test("preferred margin preserves the real benchmark's four sections", async () => {
  const cellContents = [
  ...
  const before = calculateCellBoundaries(expandedCells)
  const lines = calculateCellBoundaries(cellContents, { cellMargin: 1 })
  expect(before).toHaveLength(0)
  expect(lines).toEqual(calculateCellBoundaries(cellContents))
  expect(lines).toHaveLength(3)
  await expect({ cellContents, lines: before }).toMatchCellBoundariesSnapshot(
    import.meta.path,
    "preferred-margin-four-sections-before",
  )
  await expect({ cellContents, lines }).toMatchCellBoundariesSnapshot(
    import.meta.path,
    "preferred-margin-four-sections-after",
  )
})
